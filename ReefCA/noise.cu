...
template<typename T>
__device__ inline T ReefCA::random(int i, T step) {
...
__global__ void ReefCA::seed(T* buf, T step) {
...
template<int width, int height, int depth, typename T>
__global__ void ReefCA::seed_symmetric(T * buf, char axis, T step) {
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < width * height) {
		int x = i % width;
		int y = i / width;
...
template<int width, int height, int depth, typename T>
__global__ void ReefCA::seed_wave(T* buf, T step, int wave_m) {
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < width * height) {
		int x = i % width;
		int y = i / width;
